...
export default function QuestionList() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response =
                await fetch(`${process.env.NEXT_PUBLIC_API_URL}/question?pageNum=${page}&kw=${searchKeyword}&order=${order}&sort=${sort}`,
                    {cache: 'no-store'}
                );
        ...
        <div className="container mx-auto my-3">
            <div className="flex justify-between my-3">
                ...
                <div className="input-group">
                    ...
                    <button className="btn btn-outline-secondary" type="button" id="btn_search"
                            onClick={handleSearch}>찾기
                    </button>
